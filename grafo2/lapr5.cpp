...
void desenhaCirculo(GLfloat xi, GLfloat yi, GLfloat zi, GLfloat raio){
	GLdouble cross[3];
	GLfloat valAng=0;
	GLint n=32;
	...
	glBegin(GL_POLYGON);
		glNormal3f(0,0,1);
		for(int i=0;i<n;i++){
			glVertex3f(xi+raio*cos(valAng),yi+raio*sin(valAng),zi);
			valAng+=nAng;
		}
	glEnd();
	if(estado.apresentaNormais) {
...
void desenhaNo1(int no){
	...
	desenhaCirculo(nos[no].x,nos[no].y,nos[no].z,nos[no].largura);
	desenhaCirculo(nos[no].x,nos[no].y,nos[no].z-0.01,nos[no].largura);//desenho do chao da parte inferior...
...
void desenhaArco1(Arco arco){
	No *noi,*nof;
		...
		glPushMatrix();
		glTranslatef(noi->x,noi->y,-0.005);
		glRotatef(graus(atan2(nof->y-noi->y,nof->x-noi->x)),0,0,1);
		desenhaChao(0,-0.5*arco.largura,noi->z,noi->largura,0.5*arco.largura,noi->z, ESTE_OESTE);
		desenhaChao(noi->largura,-0.5*arco.largura,noi->z,d-nof->largura,0.5*arco.largura,nof->z, ESTE_OESTE);
		desenhaChao(d-nof->largura,-0.5*arco.largura,nof->z,d,0.5*arco.largura,nof->z, ESTE_OESTE);
		glPopMatrix();
...
void desenhaMiniMapa(int width, int height)
{
  ...
  // projec̤̣o ortogonal 2D, com profundidade (Z) entre -1 e 1
	gluOrtho2D(-30,30, -30, 30);

  // Matriz Modelview
  // Matriz onde ṣo realizadas as tranformac̵es dos modelos desenhados
...
//
	glPushMatrix();
		...
		for(int i=0; i<numNos; i++){
			glPushMatrix();
				material(preto);
				glTranslatef(nos[i].x,nos[i].y,0);
				glutSolidCube(0.5);
			glPopMatrix();
			desenhaNo1(i);
		}
