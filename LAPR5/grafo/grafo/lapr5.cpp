...
void desenhaNo(int no){
	...
	desenhaChao(nos[no].x-0.5,nos[no].y-0.5,nos[no].z,nos[no].x+0.5,nos[no].y+0.5,nos[no].z,PLANO);
	...
	if(norte)
		desenhaParede(nos[no].x-0.5,nos[no].y+0.5,nos[no].z,nos[no].x+0.5,nos[no].y+0.5,nos[no].z);
	if(sul)
		desenhaParede(nos[no].x+0.5,nos[no].y-0.5,nos[no].z,nos[no].x-0.5,nos[no].y-0.5,nos[no].z);
	if(este)
		desenhaParede(nos[no].x-0.5,nos[no].y-0.5,nos[no].z,nos[no].x-0.5,nos[no].y+0.5,nos[no].z);
...
void desenhaArco(Arco arco){
	...
	if(nos[arco.noi].x==nos[arco.nof].x){
		...
		desenhaChao(noi->x-0.5,noi->y+0.5,noi->z,nof->x+0.5,nof->y-0.5,nof->z, NORTE_SUL);
		desenhaParede(noi->x-0.5,noi->y+0.5,noi->z,nof->x-0.5,nof->y-0.5,nof->z);
		desenhaParede(nof->x+0.5,nof->y-0.5,nof->z,noi->x+0.5,noi->y+0.5,noi->z);
	}else{
		if(nos[arco.noi].y==nos[arco.nof].y){
			//arco horizontal
			if(nos[arco.noi].x<nos[arco.nof].x){
			...
			desenhaChao(noi->x+0.5,noi->y-0.5,noi->z,nof->x-0.5,nof->y+0.5,nof->z, ESTE_OESTE);
			desenhaParede(noi->x+0.5,noi->y+0.5,noi->z,nof->x-0.5,nof->y+0.5,nof->z);
			desenhaParede(nof->x-0.5,nof->y-0.5,nof->z,noi->x+0.5,noi->y-0.5,noi->z);
		}
		else{
			cout << "arco diagona... não será desenhado";
		}
	}
}
